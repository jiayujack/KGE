...
# 定义所有要运行的模型
models=('distma' 'distmult' 'distmultliteral' 'distmultliteralgated' 'ermlp' 'ermlpe' 'fixed' 'hole' 'inductivenodepiece' 'inductivenodepiecegnn' 'kg2e')
...
do
  ...
  # 运行训练命令并等待完成
  CUDA_VISIBLE_DEVICES=1 python train_classification.py \
    --model "$model" \
    --data_dir "$data_dir" \
